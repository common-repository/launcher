...
class WP_Launcher {
	    	...
	    	<form method="post" action="options.php">
	    		...
	    		<div class="wplauncher-nav-tab-contents">
	    			...
	    			<div id="wplauncher-settings-countdown" style="display: none;">
		                ...
		                <input type="hidden" name="wplauncher_options[countdown][date]" id="wplauncher_options-countdown-date" value="<?php echo esc_attr( $this->settings['countdown']['date'] ); ?>">
	    			</div>
		                    ...
		                    <?php
		                    	...
		                    	<tr class="show-if-subscribe if-aweber">
						            ...
						            <td>
										...
										<input type="hidden" id="wplauncher_options-subscribe-aweber-consumer_key" name="wplauncher_options[subscribe][aweber][consumer_key]" value="<?php echo esc_attr(isset($this->settings['subscribe']['aweber']['consumer_key']) ? $this->settings['subscribe']['aweber']['consumer_key'] :''); ?>" />
										<input type="hidden" id="wplauncher_options-subscribe-aweber-consumer_secret" name="wplauncher_options[subscribe][aweber][consumer_secret]" value="<?php echo esc_attr(isset($this->settings['subscribe']['aweber']['consumer_secret']) ? $this->settings['subscribe']['aweber']['consumer_secret'] :''); ?>" />
										<input type="hidden" id="wplauncher_options-subscribe-aweber-access_key" name="wplauncher_options[subscribe][aweber][access_key]" value="<?php echo esc_attr(isset($this->settings['subscribe']['aweber']['access_key']) ? $this->settings['subscribe']['aweber']['access_key'] :''); ?>" />
										<input type="hidden" id="wplauncher_options-subscribe-aweber-access_secret" name="wplauncher_options[subscribe][aweber][access_secret]" value="<?php echo esc_attr(isset($this->settings['subscribe']['aweber']['access_secret']) ? $this->settings['subscribe']['aweber']['access_secret'] :''); ?>" />
						           	</td>
						        </tr>
								<?php
	...
	public function template_footer() {
		...
		if ( ! empty( $this->settings['footer_code'] ) && ! LAUNCHER_DISABLE_SCRIPT_INPUT ) {
			$code = $this->settings['footer_code'];
			if ( ! defined( 'LAUNCHER_ALLOW_UNFILTERED_HTML' ) || ! LAUNCHER_ALLOW_UNFILTERED_HTML ) {
		...
		if (apply_filters( 'wplauncher_show_admin_bar', true )) {
			...
			<link rel="stylesheet" href="<?php echo esc_attr( includes_url( 'css/dashicons.min.css' ) ); ?>">
			<link rel="stylesheet" href="<?php echo esc_attr( includes_url( 'css/admin-bar.min.css' ) ); ?>">
			<script type="text/javascript" src="<?php echo esc_attr( includes_url( 'js/admin-bar.min.js' ) ); ?>"></script>
	...
	public function template_head() {
		...
		if ( ! empty( $this->settings['header_code'] ) && ! LAUNCHER_DISABLE_SCRIPT_INPUT ) {
			$code = $this->settings['header_code'];
			if ( ! defined( 'LAUNCHER_ALLOW_UNFILTERED_HTML' ) || ! LAUNCHER_ALLOW_UNFILTERED_HTML ) {
				$code = wp_kses_post( $code );
			}
			$output .= $code;
	...
	public function template_tag_countdown( $params = '' ) {
		...
		ob_start();
		?>
		<div class="wplauncher-countdown<?php echo ' '.$classes; ?>" style="<?php echo esc_attr( $style_attr ); ?>"<?php echo $color_attrs.$hideable_attrs; ?>><div id="wplauncher-countdown"></div></div>
		<script type="text/javascript">
			var wplauncher_countdown_refresh_rate = <?php echo $params['refresh_rate']; ?>;
		</script>
		<script src="<?php echo esc_attr( WPLAUNCHER_URI.'js/jquery.countdown.min.js' ); ?>"></script>
		<script type="text/javascript">
			jQuery(document).ready(function($) {
		...
		if ($this->settings['subscribe']['service'] == 'feedburner') {
			...
			<form id="wplauncher-subscribe" class="wplauncher-feedburner-form" method="post" action="http://feedburner.google.com/fb/a/mailverify" target="wplwindow">
				<fieldset class="wplauncher-subscribe-fieldset">
					<input name="wplauncher-subscribe-email" id="wplauncher-subscribe-email" type="text" placeholder="<?php echo esc_attr( $this->settings['subscribe']['email_label'] ); ?>"/>
	...
	public function options_field_text($name, $label, $description = '', $attrs = false, $row_class = '') {
		...
		?>
		<tr class="<?php echo esc_attr( $attrs['id'].' '.$row_class ); ?>">
			<th><label for="<?php echo esc_attr( $attrs['id'] ); ?>"><?php echo wp_filter_post_kses( $label ); ?></label></th>
			<td>
	...
	public function options_field_checkbox($name, $label, $description = '', $attrs = false, $row_class = '') {
		...
		<tr class="<?php echo esc_attr( $attrs['id'].' '.$row_class ); ?>">
			...
			<td>
				...
				<?php } ?>
				<input type="hidden" name="<?php echo esc_attr( $attrs['name'] ); ?>" value="0">
				<input<?php echo $attributes; ?>>
				<?php if (!empty($description)) { ?>
					<p class="description"><?php echo wp_filter_post_kses( $description ); ?></p>
				<?php } ?>
	...
	function options_field_select( $name, $options, $label, $description = '', $attrs = false, $row_class = '' ) {
		...
		?>
		<tr class="<?php echo esc_attr( $attrs['id'].' '.$row_class ); ?>">
			<th><label for="<?php echo esc_attr( $attrs['id'] ); ?>"><?php echo wp_kses_post( $label ); ?></label></th>
			<td>
		...
		if ($_POST['type'] == 'mailchimp') {
			...
			if (!isset($this->settings['subscribe']['mailchimp']['api_key']) || $this->settings['subscribe']['mailchimp']['api_key'] == '') {
				echo json_encode(array(
					'status'	=> 'warning',
					'message'	=> __('MailChimp account is not setup properly.', 'wplauncher'),
				));
			...
			if (!isset($this->settings['subscribe']['mailchimp']['list']) || $this->settings['subscribe']['mailchimp']['list'] == '') {
				echo json_encode(array(
					'status'	=> 'warning',
					'message'	=> __('MailChimp: No list specified.', 'wplauncher'),
				));
